...
def gerar_secao_comissoes(story, dados_processo_formatado: dict):
    ...
    formula_adiant = Paragraph(
        "<b>Adiantamento:</b> Comissão = Valor Pago × TCMP × 1,0", STYLE_CORPO
    )
    story.append(formula_adiant)

    formula_regular = Paragraph(
        "<b>Regular:</b> Comissão = Valor Pago × TCMP × FCMP", STYLE_CORPO
    )
    ...
    if adiantamentos:
        ...
        dados_tabela.append(
            [
                Paragraph("<b>TOTAL</b>", STYLE_DESTAQUE),
                "",
                "",
                Paragraph(
                    f"<b>{formatar_moeda(total_valor_adiant)}</b>", STYLE_DESTAQUE
                ),
                Paragraph(f"<b>{formatar_moeda(total_adiant)}</b>", STYLE_DESTAQUE),
            ]
        )

        larguras = [140, 100, 70, 100, 100]
        tabela = TableBuilder.criar_tabela_resumo(
            dados_tabela, larguras=larguras, linhas_destaque=[len(dados_tabela) - 1]
        )
        if tabela:
            story.append(tabela)
    ...
    if regulares:
        ...
        dados_tabela = [
            ["Colaborador", "Cargo", "TCMP", "FCMP", "Valor Pago", "Comissão"]
        ]
        ...
        dados_tabela.append(
            [
                ...
                "",
                Paragraph(
                    f"<b>{formatar_moeda(total_valor_regular)}</b>", STYLE_DESTAQUE
                ),
                Paragraph(f"<b>{formatar_moeda(total_regular)}</b>", STYLE_DESTAQUE),
            ]
        )
    ...
    gerar_paragrafo(
        story, f"<b>TOTAL GERAL DE COMISSÕES:</b> {estatisticas['total_comissoes']}"
    )
